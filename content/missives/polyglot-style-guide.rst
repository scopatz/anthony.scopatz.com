Polyglot Style Guide
#######################
:date: 2014-02-02 10:35
:author: Anthony Scopatz
:category: missives
:slug: polyglot-style-guide

Today the pyne community `approved a pull request <https://github.com/pyne/pyne/pull/257>`_ 
for `a new style guide <https://github.com/pyne/pyne/blob/staging/docs/devsguide/style_guide.rst>`_ 
I wrote.  Writing this went against every bone in my body. You are supposed to use
existing style guides. You are supposed to trust that the authors of other style 
guides know what they are doing. Or barring their competence, that they are at least
consistent.  These assumptions work great as long as 

1. such a style guide exists, and
2. you live in only one language.

PyNE provides first class APIs in three languages (C++, Python, & Cython) with 
more to come, in all likelihood. Other projects rely on pyne's API in all languages 
and users see pyne from all levels.  We are not allowed to hide behind bad design 
because it is 'just an optimization' or it is for pyne developers only.  We need pyne
to be nice everywhere.  

No single style guide I have found really covers the multi-language scenario.  
Sure, CPython has PEP7 as a corollary to PEP8, but it falls under the not-for-users
category. Many projects use different style guides for different languages saying, for
example, "use PEP8 for Python and the Google C++ Style Guide for C++."  However, 
the results of this are weird. Different guides make different assumptions about 
fundamental abstractions: how are types names, how should memory be managed, and 
so on.  

In the case of Cython, no style guide actually existed and folks just do 
something PEP8-ish.  This really isn't enough.  I hope that this is a stepping stone
for a more formal guide for Cython eventually.

In the pyne style guide, I tried to bring consistency across the language 
barriers while remaining idiomatic to the individual languages.  I also tried to 
rely on existing style guides, deferring to or forking from them as needed. I also 
tried to tie together the languages with some head matter that I hope puts the reader
in the pyne mindset and is good advice no matter what language you are coding in.

I have included the style guide here in its current form.  Some of it is a bit 
nuclear engineering specific.  Enjoy!
